...
BUILD_PACKAGE_NAME=github.com/pseudo-su/morningpaper2remarkable
BUILD_VERSION := $(shell cat VERSION.txt)
BUILD_GITCOMMIT := $(shell git rev-parse --short HEAD)
BUILD_VERSION_FLAG=-X '$(BUILD_PACKAGE_NAME)/internal.VERSION=$(BUILD_VERSION)'
BUILD_GITCOMMIT_FLAG=-X '$(BUILD_PACKAGE_NAME)/internal.GITCOMMIT=$(BUILD_GITCOMMIT)'
BUILD_LDFLAGS=-ldflags="$(BUILD_VERSION_FLAG) $(BUILD_GITCOMMIT_FLAG)"

## Build binary
build:
	mkdir -p ./build
	env CGO_ENABLED=0 go build $(BUILD_LDFLAGS) -o build/morningpaper2remarkable ./cmd/morningpaper2remarkable/main.go
.PHONY: build
